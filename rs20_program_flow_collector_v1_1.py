...
- App Key / Secret are read from existing Windows environment variables.
...
OUT_MATCHED = "rs20_program_flow_collection_status_v1_1.csv"
OUT_MISSING = "rs20_program_flow_missing_v1_1.csv"
OUT_PROGRESS = "rs20_program_flow_progress_v1_1.csv"
OUT_SUMMARY = "rs20_program_flow_collection_summary_v1_1.txt"
...
# Existing project environment-variable names are checked first.
# Several common aliases are supported so credentials do not need to be
# pasted into this collector or committed to Git.
APPKEY_ENV_NAMES = [
    "KIWOOM_APP_KEY",
    "KIWOOM_APPKEY",
    "KIWOOM_REST_APP_KEY",
...
def issue_access_token(session):
    ...
    if not appkey or not secretkey:
        print()
        print("KIWOOM App Key / Secret environment variables were not found.")
        print("Checked App Key names :", ", ".join(APPKEY_ENV_NAMES))
        print("Checked Secret names  :", ", ".join(SECRET_ENV_NAMES))
        print()
        print("No credential value was printed.")
        print("If your existing project uses different variable names,")
        print("show me ONLY the variable names, never their values.")
        raise SystemExit(2)

    print(f"AUTH ENV                : {app_name} / {sec_name}")
...
def main():
    ...
    print("=" * 94)
    print("RS20 PROGRAM_FLOW COLLECTOR v1.1")
    ...
    try:
        token = issue_access_token(session)
    except Exception as e:
        session.close()
